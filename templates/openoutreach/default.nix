...

{
  ...
  systemd.services.x11vnc = {
    ...
    serviceConfig = {
      ExecStart = "${pkgs.x11vnc}/bin/x11vnc -display :99 -listen 127.0.0.1 -rfbport 5900 -shared -forever -nopw";
      Restart = "always";
      RestartSec = "2s";
    };
  };
  ...
  virtualisation.oci-containers.containers.openoutreach = {
    ...
    cmd = [
      "bash" "-c"
      "echo \"CSRF_TRUSTED_ORIGINS = ['https://*.scape.missionctrl.dev']\" >> /app/linkedin/django_settings.py && python manage.py migrate --no-input && python manage.py setup_crm && python manage.py runserver 0.0.0.0:8000 & python manage.py"
    ];
  };
